...
1.  Enter `rc_loannumber eq` for Filter rows and select **LoanNumber** from the dynamic content pane.
    ...
    > Note: If you do not see the **Dynamic Content & Expression** pop-up, increase your browser width or zoom out slightly to reveal it (Keyboard shortcut: **Ctrl + -**).

1.  Add single quotes (') before and after LoanNumber. Make sure there is a spca between `eq` and `'`.
...
1.  Paste the expression below and click **OK**. This checks if any rows were returned.
...
1.  Click to select the second operand field, go to the dynamic content pane, and select the expression tab.
...
1.  Click to select the value field, go to the dynamic content pane, and select the **Expression** tab.

1.  Paste the expression below and click **OK**. This gets the ID from the first row.
...
1.  Navigate to <https://make.powerapps.com/> and make sure you are in the **Dev One** environment.
...
1.  Provide your **virtual machine username**, **password**, and click **Create**. If your username doesn’t work try admin as the username.
...
1.  Navigate to <https://make.powerapps.com/> and make sure you are in the **Dev One**
    environment.
...
1.  Enter `Work`. You should now have three input parameters. Click **+ New step**.
...
1.  Click **Continue**.
    ...
    ![run the flow](media/41eb830c1d4da67216e618327a2805e2.png)
...
1.  Select **Apps** and launch the **Loan Manager** application by hovering over it and selecting **Play**.
...
1.  Enter your name for **Username**, and `pass@word1` for **Password**.
...
1.  Enter `JG7165` for **LoanNumber**, paste the id you copied in the previous
    ...
    ![run the flow with the provided details](media/dc027ac00907533066cc4fbb2af45e22.png)
...
1.  Navigate to <https://admin.microsoft.com/> and click **Show all**.
    ...
    ![in the admin portal, select show all](media/deed82ec8c0efea0fb41c7f21be2ec8e.png)
...
1.  Enter **Process Construction Funding Request** for Flow name, select the **When a new email arrives in a shared mailbox (V2)** trigger, and click **Create**.
...
1.  Select **Yes** for Include attachments and click on the **…** button of the trigger.
...
1.  Click on the **LoanNumber** field, go to the dynamic content pane, and select **Subject**.

    ![enter details as described](media/100a3a1bee9065b8e17a6b110e397184.png)

1.  Rename the step as `Lookup loan number`.

1.  Click **+ New step**.

1.  Select the **Condition** action from the **Control** connector.

1.  Click to select the first operand field, go to the dynamic content pane, and select **LoanFound**.
...
1.  Rename the condition to `Check if loan number found`.
...
1.  Rename the reply to `Send loan not found email` and then click **Add an action**.
...
1.  Type `rc_loans()` for Loan (Loans), place your cursor inside the parentheses, and select **LoanID** from the dynamic content pane.

    ![enter details as described](media/105bb2261e9191687c0bdafe02361576.png)

1.  Rename the step to `Create new loan draw row`.
...
1.  Click on the **LoanDrawID** field and select **Loan Draw** from the dynamic content pane.

    ![enter details as described](media/3795b1d1da9b36ba5867f52c2c2336e2.png)

1.  Enter **Test work item** for Work.

1.  Rename the child flow to `Run inspection process` and click **+ New step**.
...
1.  Click on the **LoanNumber** field and select **Subject** from the dynamic content pane.

1.  Click on the **LoanDrawID** field and select **Loan Draw** from the dynamic content pane.

1.  Enter `80000` for **RequestedAmount**.

1.  Click on the **InspectionJobID** field and select **JobNumber** from the dynamic content pane.

1.  Enter `55` for **RiskScore**.

1.  Rename the child flow to `Run funding process`. Click **Save**.
...
1.  Click on the first operand and select **FundingStatus** from the dynamic content pane.
...
1.  Rename the condition to `Check approval status`.
...
1.  Click on the **Original Mailbox Address** field, click **Add dynamic content** and select **To** from the dynamic content pane.

1.  Click on the **To** field, click **Add dynamics content** and select **From** from the dynamic content pane.

1.  Enter `Draw Approved` for Subject.

1.  Type `Your draw for` in the Body and select **Amount Funded** from the dynamic content pane.
...
1.  Click on the **To** field, click **Add dynamic content** and select **From** from the dynamic content pane.
...
1.  Rename the step to `Send draw not approved email`.
...
1.  Send an email with the subject `MC3747` from your personal email to the Funding shared email you created ([Funding@yourdomain.onmicrosoft.com](mailto:Funding@yourdomain.onmicrosoft.com)).

1.  Wait for the flow to get triggered. Do not interact your computer while the flow is running.
